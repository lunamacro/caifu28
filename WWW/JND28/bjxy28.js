...
var Lottery =
{
    LotID: 0,  //采种编号
    LotName: "",  //彩中名称
    LotTypeID: 9801,  //彩种分类，1 乐透型（分前后区，如双色球、大乐透） 2 乐透型（单区，如22选5 36选7） 3 排列型（如：3D、排3/5、七星彩、时时彩等）
    PlayTypeID: 0,  //玩法编号
    PlayTypeName: "",  //玩法名称
    ...
    TotalMoney: 0,  //总金额
    TotalMultiple: 1, //总倍数
    MaxAllowInvestMultiple: 989999, //允许投注的最大倍数,默认999倍
        ...
        $(".balls li b,.balls li a,.balls li em b").click(function () {
            ...
            if (Lottery.BallStatus($(this))) {
                ...
                if ($.inArray(Lottery.LotteryNumber, Lottery.DaXiaoContentList) >= 0) {
                    ...
                    Lottery.PlayTypeID = "9801";
                    buyParameter.PlayTypeID = "9801";
                } else if ($.inArray(Lottery.LotteryNumber, Lottery.TeShuContentList) >= 0) {
                    Lottery.PlayTypeName = "特殊玩法";
                    Lottery.PlayTypeID = "9803";
                    buyParameter.PlayTypeID = "9803";

                } else {
                    Lottery.PlayTypeName = "猜数字";
                    Lottery.PlayTypeID = "9802";
                    buyParameter.PlayTypeID = "9802";
                }
            }
    ...
    Submit: function () { //提交投注
        ...
        if (buyParameter.IsChase == 1) { //追号
            ...
            //彩金计算
            var handAmount = parseFloat(JND28_Default.GetHandselAmount().value);
